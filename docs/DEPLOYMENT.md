...
서버가 처음 뜰 때 이 계정이 자동 생성된다(`createsuperuser --noinput`). 이 계정 하나를 감지 페이지와 보호자 앱이 함께 쓴다.
...
2. `https://<아이디>.github.io/<저장소>/`에 접속 → 2단계에서 만든 계정으로 로그인 → 방 선택 → 감지 시작 → 카메라 허용. GitHub Pages는 https라서 카메라 권한이 정상 동작한다.
