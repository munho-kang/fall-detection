...
코드와 자동 테스트(backend JUnit 48 / web Vitest 34 / app 23)는 전부 통과한 상태다.
...
- [ ] 카메라 앞에 서면 **"지금은 안전해요"**(초록), 프레임을 벗어나 2초 지나면 **"사람이 보이지 않아요"**(회색)
- [ ] 세 관문 중 ②의 값이 서 있을 때 0~15°, 누우면 70~90°
- [ ] 천천히 눕는다 → 관문 ①이 열리지 않는다(**"넘어지는 중"**으로 가지 않는다)
- [ ] 빠르게 눕고 5초 버틴다 → **"넘어지는 중"** → **"쓰러짐 — 지켜보는 중"** → **"낙상 확정"**(빨강). 세 관문이 ①②③ 순서로 열리고 화면 전체가 붉게 물든다
...
감지 화면의 세 관문 값은 30fps로 갱신되고 빠른 낙상은 300ms(≈9프레임) 만에 끝난다. **눈으로 최대치를 읽는 것은 불가능하다.** 게다가 그 순간 본인은 바닥에 누워 있다. 그래서 튜닝 패널을 따로 뒀다 (`web/js/tuning.js`). 시연 화면을 깨끗하게 두려고 평소엔 숨겨 두었으니 **`d` 키**로 연다.
...
**이 절은 자동화됐다.** 아래 명령이 앱 5개를 전부 밟는다. 준비 절의 백엔드(8000)가 떠 있어야 한다. 실행할 때마다 새 계정을 만들어 쓰므로 기존 데이터를 건드리지 않는다.
...
앱 통합 테스트는 서버가 있어야 돌기 때문에 `flutter test`(단위 7건)에는 섞이지 않는다.
...
- [ ] 빠르게 넘어져 FALLEN이 되는 순간 **응급 대응 타임라인**이 나타나고 상태가 **"대답을 듣는 중"**이 된다
- [ ] 10초에 스피커에서 **"괜찮으세요?"**가 나오고 상태가 **"괜찮으세요? 확인 중"**으로 바뀐다 (타임라인의 두 번째 점이 채워진다)
- [ ] 그대로 침묵하면 20초에 **"119 자동 신고됨"**이 뜨고 타임라인 세 점이 모두 채워진다 (넘어진 순간부터 20초다 — 질문 후 20초가 아니다)
- [ ] (일어나서 재무장 후 새 에피소드) 질문이 나온 뒤 "괜찮아"라고 답하면 **"응답 확인 — 신고 안 함"**이 뜨고 신고가 나가지 않는다
- [ ] (새 에피소드) 질문 전(10초 안)에 "괜찮아" → **질문 없이** 바로 해제된다
- [ ] (새 에피소드) 도중에 일어나면 타임라인이 사라지고 신고도 질문도 없다
