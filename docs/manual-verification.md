...
코드와 자동 테스트(backend JUnit 39 / web Vitest 22 / app 4)는 전부 통과한 상태다.

**7절은 나중에 자동화됐다.** 시뮬레이터와 실제 Chrome을 구동하는 E2E가 그 절 대부분을 대신 밟는다. 자세한 것은 7절 머리말에 있다.
...
웹 E2E는 브라우저를 내려받지 않고 시스템에 깔린 Chrome을 쓴다(`channel: "chrome"`). 앱 통합 테스트는 서버가 있어야 돌기 때문에 `flutter test`(단위 4건)에는 섞이지 않는다.
