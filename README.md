...
- **임계값은 카메라 배치와 한 세트다** — 2026-07-17에 20시행 8899프레임을 실측해 `CONFIG` 값을 검증했다. 넘어지기 5/5 감지, 눕기·앉기·서 있기 11/11 오탐지 없음. 다만 `hipVelocity`는 정규화 좌표 기준이라 **물리적 낙하 속도가 아니라 화면상 겉보기 속도**를 잰다. `0.45/s`는 물리 상수가 아니라 측정 당시 카메라 배치에서만 유효한 숫자이고, 카메라를 옮기면 튜닝이 통째로 무효가 된다. 유효 구간은 0.381~0.515로 넓지 않다.
