...
목록·홈 타일에는 상태 배지가 붙는다(`119 신고됨` > `괜찮다고 말함` > `확인함`/`미확인`). 앱은 기기 다크모드와 무관하게 항상 라이트 테마로 그려진다. 화면 캡쳐는 `docs/screenshots/`에 있다.
...
cd app     && flutter test      # 64개 — 화면·모델·폴링·알림·사고 발생 창·전화 발신
```
