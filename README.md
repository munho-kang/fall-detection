...
- **119는 실제 발신이 아니다** — 수동 버튼(알림 상세 화면·사고 발생 창)은 더미 번호(`app/lib/dial.dart`의 `emergencyPhone`)를 쓰고, 음성 무응답 자동 신고(2026-07-27 추가)도 발신 없이 `reported_119_at` 기록과 화면 표시까지다. 시연 중 실제 신고를 막기 위함이다.
