...
- **카메라 각도에 유효 대역이 있고, 양쪽 끝이 다 조용히 실패한다** — 이 시스템의 가장 취약한 지점이다. **너무 수평**이면 몸통 축이 카메라 광축과 일직선이 될 때 투영이 짧아져 바닥에 누워도 tilt가 0°에 가깝게 나온다(관문 2 실패). **너무 수직(내려다봄)**이면 수직 하강이 광축 방향이라 엉덩이가 70cm 떨어져도 화면에서 안 움직인다(관문 1 실패). 실제로 측정 중 노트북을 다시 올려놓은 것만으로 후자에 빠져 **낙상 3/3 미탐지**가 났다. 낙상 중 화면상 하강량이 정상 배치 0.233~0.284에서 -0.029~0.032로 죽었다. 둘 다 알림이 그냥 안 올 뿐 에러가 없어서 눈치채기 어렵다.
- **카메라를 옮기면 반드시 아래 두 검수를 다시 통과시켜야 한다** — 높이·거리 수치보다 이 합격 기준이 우선이다. ① 빠르게 넘어져서 튜닝 패널의 **hipV 피크가 0.5 이상**일 것(실측 낙상 5회는 0.515~0.641). 못 넘으면 너무 내려다보는 것이다. ② 옆·정면·대각선으로 누워 **tilt가 셋 다 60° 이상**일 것(실측 88° 이상). 못 넘는 방향이 있으면 너무 수평이다.
...
- **현재 유효 배치의 오탐지 쪽은 미측정** — `FALLEN_HOLD` 5초 경로는 2026-07-17 3차 측정에서 실측으로 검증됐다. 넘어져 5초 유지 4회가 ALERTED→POST로 이어져 DB에 4건 도달했고, 4.6초에 일어난 1회는 알림이 안 나갔다. 다만 그 배치(높이 150 / 거리 180 / 각도 83)에서는 넘어지기만 재고 눕기·앉기를 안 재서, 0.45가 이 배치의 오탐지 쪽과 얼마나 떨어져 있는지는 확인하지 못했다. 넘어지기 관문 창 hipV 최솟값이 0.456으로 임계값 0.45에 바짝 붙어 여유가 얇다.
