...
임계값은 2026-07-17에 20시행 8899프레임을 실측해 검증했다 — 넘어지기 5/5 감지, 눕기·앉기·서 있기 11/11 오탐지 없음. 상수는 `web/js/detector.js`의 `CONFIG` 한 곳에 모여 있어 카메라 배치에 맞춰 조정할 수 있고, `web/js/tuning.js`의 튜닝 패널이 hipV·tilt 실측값을 실시간으로 보여준다(시연 화면을 깨끗하게 두려고 평소엔 숨어 있고 `d` 키로 연다). 측정 원본은 `docs/tuning/`에 CSV로 남아 있다.
...
Mac에서 여는 관제 화면이다. 왼쪽은 검은 배경 위 스켈레톤, 오른쪽은 판정 패널이다.

- **판정** — 상태를 한국어 한 줄로 크게 띄운다(`지금은 안전해요` · `넘어지는 중` · `쓰러짐 — 지켜보는 중` · `낙상 확정`). 색이 함께 바뀐다: 안전=초록 · 주의=주황 · 낙상=빨강
- **세 관문** — 위 알고리즘의 세 관문이 그대로 세 개의 막대다. 각 막대는 현재 측정값과 기준선(70% 지점의 눈금)을 함께 보여주고, 열린 관문은 `✓ 통과`가 붙는다. 판정 근거가 화면에 그대로 남는다
- **응급 대응** — 낙상이 확정되면 `보호자 알림 5초 → 괜찮으세요? 10초 → 119 신고 20초` 타임라인이 나타나고, 다음 단계까지 남은 초를 센다
- **낙상 순간** — 화면 전체가 붉게 물들고 상단 바도 빨강으로 바뀐다. 멀리서도 사고를 알아본다
- 튜닝 패널은 `d`, 시행 끊기는 `r` 키다

색은 보호자 앱과 같은 의미 체계(초록=안전, 빨강=긴급)를 쓰고 글꼴도 같은 Pretendard다. 어두운 바탕은 스켈레톤이 검은 화면 위에 그려지기 때문이다.
