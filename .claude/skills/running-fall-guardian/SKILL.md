...
코드는 `detect.html`로 이동한다(`web/index.html`). 확장자가 떨어져 나간다면 브라우저 프로파일 쪽 문제다 — 포트 5500은 VS Code Live Server 기본 포트라 다른 프로젝트가 같은 오리진에 서비스 워커를 남겨뒀을 수 있고, `web/sw.js`에는 `skipWaiting()`이 없어 옛 워커가 계속 제어권을 쥔다. URL을 정리하는 확장 프로그램도 같은 증상을 낸다.

시크릿 창에서 정상이면 둘 중 하나다. `chrome://serviceworker-internals/`에서 `127.0.0.1:5500` 등록을 해제하고 하드 리로드해서 갈라낸다.
