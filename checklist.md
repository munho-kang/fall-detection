...
- [x] 웹캠 권한 거부 처리
- [x] 모델 로드 실패 처리
- [ ] 브라우저에서 넘어져 보고 상태 전이 관찰 — 수동 검증 대기 (웹캠 앞 실측 필요, Task 16 범위 밖)
...
- [ ] 실제로 넘어지면 DB에 행 생성 확인 — 수동 검증 대기 (서버 기동 + 웹캠 필요, Task 16 범위 밖)
...
- [ ] 실제 웹캠으로 임계값 튜닝 (조정 내역은 context-notes.md에) — 미실시, 사람의 수동 실측이 필요해 이번 세션에서 하지 않음 (context-notes.md의 "실측 미실시" 항목 참고)
- [ ] 오탐지 없이 3회 연속 감지 성공 — 미실시, 위 튜닝이 선행돼야 함
